...
export function createDemoActivityData({ weeks = 20, to = "2026-09-07", username = "your-handle", } = {}) {
    ...
    for (let current = start; current.getTime() <= end.getTime(); current = addDays(current, 1)) {
        ...
        const contributions = githubRoll < 0.22 ? 0 : Math.max(1, Math.round(noise(index, 2) * 9 * weekendDip));
        githubDays.push({ date, contributions });
        const providers = {};
        if (noise(index, 3) > 0.28) {
            providers.claude = Math.round((3 + noise(index, 4) * 26) * weekendDip) * 1000000;
        }
        if (noise(index, 5) > 0.34) {
            providers.codex = Math.round((2 + noise(index, 6) * 17) * weekendDip) * 1000000;
        }
