...
import { CURSOR_METRICS, defaultCursorDbPaths, scanCursorLogs } from "./cursor.mjs";
...
/**
 * How Cursor may be measured. "auto" prefers tokens when OpenUsage is running
 * (it reads Cursor's cloud usage API, which reports real tokens) and falls
 * back to the local database, which only records messages and edited lines.
 */
const CURSOR_METRIC_SETTINGS = ["auto", "tokens", ...CURSOR_METRICS];
...
/**
 * A provider's series must hold exactly one unit. When its unit changes —
 * OpenUsage starts supplying Cursor tokens where the local database was
 * supplying message counts — the values already stored are in the old unit
 * and cannot be merged with the new ones, so they are dropped.
 */
/**
...
export async function syncActivity({
  ...
  // OpenUsage first: it reads each vendor's cloud usage API, so it is the only
  // local source that reports Cursor in tokens. Whether it has Cursor data
  // decides which unit the Cursor series uses, so it has to resolve before any
  // Cursor days are ingested.
  let openUsageDays = null;
  if (config.sources.openUsage) {
    try {
      openUsageDays = await fetchOpenUsage({
  ...
  const openUsageHasCursor = (openUsageDays?.cursor?.size ?? 0) > 0;
  const cursorMetric =
    config.cursorMetric === "auto"
      ? openUsageHasCursor
        ? TOKEN_METRIC
        : "messages"
      : config.cursorMetric;
  ...
  if (config.sources.cursor) {
    if (config.cursorFilePath) {
      // An explicit export wins over every automatic source.
      const cursorDays = await readCursorDays(config.cursorFilePath, range);
      scans.cursor = { provider: "cursor", source: "cursor export", days: cursorDays, metric: cursorMetric };
      setProviderDays(historyDaysByDate, metrics, "cursor", cursorDays, cursorMetric);
    ...
    } else if (cursorMetric === TOKEN_METRIC) {
      if (openUsageHasCursor) {
        scans.cursor = {
          provider: "cursor",
          source: "openusage-local",
          days: openUsageDays.cursor,
          metric: TOKEN_METRIC,
        };
        setProviderDays(historyDaysByDate, metrics, "cursor", openUsageDays.cursor, TOKEN_METRIC);
        const dates = Array.from(openUsageDays.cursor.keys()).sort();
        coverage.cursor = mergeCoverage(coverage.cursor, {
          from: dates[0], to: dates.at(-1), source: "openusage-local", complete: false,
        });
        sources.cursor = "openusage-local";
      } else {
        const message =
          "Cursor tokens need OpenUsage running; set cursorMetric to \"auto\", \"messages\" or \"edits\"";
        warnings.push(message);
        logger.warn(message);
      }
    } else {
      const cursor = await scanCursorLogs({ homeDir, timeZone: config.timezone, metric: cursorMetric });
      scans.cursor = cursor;
      if (!cursor.available) {
  ...
  if (openUsageDays) {
    const openUsageCoverage = coverageFromProviderDays(openUsageDays, "openusage-local");
    // Cursor is handled above so its unit is decided in one place.
    DEFAULT_TOKEN_PROVIDERS.forEach((provider) => {
      setProviderDays(historyDaysByDate, metrics, provider, openUsageDays[provider], TOKEN_METRIC);
...
async function firstCursorDb(homeDir) {
  for (const candidate of defaultCursorDbPaths(homeDir)) {
    try {
...
export async function inspectEnvironment({ cwd = process.cwd(), homeDir = os.homedir(), env = process.env } = {}) {
  ...
  return {
    ...
    sources: {
      ...
      cursorDb: await firstCursorDb(homeDir),
