...
/**
 * Only these providers report tokens, so only these contribute to a day's
 * totalTokens. Cursor's messages/edits stay in its own provider bucket and
 * are never folded into a token total.
 */
const TOKEN_PROVIDERS = ["claude", "codex"];

function sumTokenProviders(providers) {
  return TOKEN_PROVIDERS.reduce((sum, provider) => sum + (numericValue(providers?.[provider]) || 0), 0);
}
...
function resolveConfig(cwd, rawConfig = {}, env = process.env) {
  ...
  return {
    ...
    cursorMetric: CURSOR_METRICS.includes(env.ACTIVITY_CURSOR_METRIC || merged.cursorMetric)
...
function addProviderDays(daysByDate, providerDays, provider) {
  const contributesTokens = TOKEN_PROVIDERS.includes(provider);
  providerDays.forEach((value, date) => {
    ...
    daysByDate.set(date, mergeAiDay(existing, {
      date,
      totalTokens: contributesTokens ? value : 0,
...
export async function syncActivity({
  ...
  if (config.sources.codex) {
    ...
    if (dates.length > 0) {
      ...
      metrics.codex = TOKEN_METRIC;
    }
  }

  if (config.sources.cursor) {
    // An explicit cursorFile export wins; otherwise read Cursor's own database.
    if (config.cursorFilePath) {
      const cursorDays = await readCursorDays(config.cursorFilePath, range);
      scans.cursor = { provider: "cursor", source: "cursor export", days: cursorDays, files: 1 };
      ...
      if (dates.length > 0) {
        ...
        metrics.cursor = config.cursorMetric;
      }
    } else {
      const cursor = await scanCursorLogs({
        homeDir,
        timeZone: config.timezone,
      ...
      if (!cursor.available) {
        ...
        addProviderDays(historyDaysByDate, cursor.days, "cursor");
    ...
    try {
      ...
      PROVIDERS.forEach((provider) => {
        ...
        if (openUsageCoverage[provider]) {
          ...
          if (TOKEN_PROVIDERS.includes(provider)) metrics[provider] = TOKEN_METRIC;
        }
      });
    } catch (error) {
