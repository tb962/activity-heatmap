...
const LABEL = "dev.activity-heatmap.sync";
...
function plistFor({ cwd, cliPath, nodePath, logDir }) {
  ...
  return [
    ...
    "<key>StandardOutPath</key>" + xmlString(path.join(logDir, "activity-heatmap.log")),
    "<key>StandardErrorPath</key>" + xmlString(path.join(logDir, "activity-heatmap.error.log")),
    "</dict>",
    "</plist>",
  ].join("\n");
}
